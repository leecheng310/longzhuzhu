...
async function getCookie() {
    ...
    if (app == null || app === undefined) {
        $.msg(TITLE, "", `${app.name} 上传Cookie失败, 未匹配到域名${host}`);
        return
    }
    doParams(app);
}
...
function doJdParams() {
    ...
    if (cookie.match(/(pt_key=.+?pt_pin=|pt_pin=.+?pt_key=)/)) {
        cookie = cookie.match(/pt_key=.+?;/) + cookie.match(/pt_pin=.+?;/);
    }
    ...
    console.log(`income jd cash request body = ${body}`)
    if (body == null || body == undefined || body == '' || body.indexOf("city_withdraw") === -1 ) {
        return []
    }


    let jdCkKey = "long_ck_key";
    let cityCashKey = "long_city_cash_key"
    $.setdata(jdCkKey, cookie);
    $.setdata(cityCashKey, body);
}
